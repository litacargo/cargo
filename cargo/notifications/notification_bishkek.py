...
from .config import BISHKEK_API_URL
from .headers import HEADERS

logger = logging.getLogger(__name__)

@shared_task
def send_notification_bihskek(data_dict: dict):
    notifications = []
    ...
    if notifications:
        try:
            response = requests.post(BISHKEK_API_URL, headers=HEADERS, json=notifications)
            if response.status_code == 200:
                return "Уведомления о статусе 'Бишкек' успешно отправлены."
            else:
